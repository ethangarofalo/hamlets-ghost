...
This document defines how `Theron` should enter Hamlet's Ghost as a bounded external generator.

The goal is:
- let Theron generate artifacts for the research engine
- keep chat-specific complexity outside the core backend
...
- a bounded external generator behind a local bridge

That means the lab talks to a local adapter.
The adapter may talk to Theron through a local OpenClaw identity or, if absolutely necessary, through Telegram.

## Recommended Topology

```text
Hamlet's Ghost -> local OpenClaw/Theron bridge -> Theron local agent runtime
```

Fallback topology:

```text
Hamlet's Ghost -> local Theron gateway -> Telegram bot conversation -> gateway response parser -> lab
```

The lab should never talk to Telegram directly from `server.py`, and Telegram should remain optional rather than foundational.

The bridge should own:
- message formatting when needed
- request tracking
- polling / wait logic
- response parsing
- timeout handling
...
```bash
cd hamlets-ghost
./.venv/bin/python scripts/theron_doctor.py
```

With `THERON_BACKEND=openclaw_local`, the lab uses Theron’s local OpenClaw identity directly and does not need Telegram at all.

Telegram fallback path:

```bash
cd hamlets-ghost
./.venv/bin/python scripts/generate_theron_session.py
./.venv/bin/python scripts/theron_doctor.py
./scripts/start_theron_gateway.sh
```

That gives the lab:
- a Telegram user session string for Theron transport
- a quick health/readiness check
- a local gateway process the paired packet runner can call
...
- Athena and Apollo score both when available
...
- Muse/Athena/Apollo judgments expose useful differences
